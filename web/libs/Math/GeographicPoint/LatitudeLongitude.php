...
class LatitudeLongitude extends GeographicPoint
{
    ...
    public function __construct($latitude, $longitude = '', $datum = '')
    {
        ...
        if (!$longitude) {
            ...
            } else if (preg_match('/([a-z])(\s*)?,/i', $latitude, $delimiter)) {
                $latitude = str_replace(' ', '', $latitude);

                list($latitude, $longitude) = explode(($delimiter[1].','), $latitude);

                $latitude .= $delimiter[1];
            } else {
                list($latitude, $longitude) = explode(' ', $latitude);
            }
        }

        list($latitude, $longitude) = $this->parseDegrees($latitude, $longitude);
    ...
    public function parseDegrees ($latitude, $longitude) {
        $latitude = str_replace(',', '.', trim($latitude));
        $longitude = str_replace(',', '.', trim($longitude));

        if (preg_match('/[a-z]/i', $latitude)) {
            $matches = preg_split('/[^\w\.]+/', $latitude);

            if (count($matches) === 4) {
                $latitude = ($matches[0] + ($matches[1] / 60) + ($matches[2] / 3600)) * ((strtolower($matches[3]) === 's') ? -1 : 1);
            }
        }

        if (preg_match('/[a-z]/i', $longitude)) {
            $matches = preg_split('/[^\w\.]+/', $longitude);

            if (count($matches) === 4) {
                $longitude = ($matches[0] + ($matches[1] / 60) + ($matches[2] / 3600)) * ((strtolower($matches[3]) === 'e') ? 1 : -1);
            }
        }
   ...
   /**
    ...
    public function toTM($LongOrigin = null)
    {
        ...
        return new UTM($utmEasting, $utmNorthing, $data, $this->datum);
    }
     ...
     * @return object
     */
    public function toLambert(Lambert\Config $config)
    {
        if (!$config->isValid()) {
        ...
        return new Lambert($this->lccEasting, $this->lccNorthing, $config);
    }
