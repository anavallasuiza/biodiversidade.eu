...
class UTM extends GeographicPoint
{
    /**
    ...
    public function __construct($easting, $northing, $zone, $datum = '')
    {
        ...
        if (is_array($zone)) {
            ...
            if (preg_match('/^[0-9]+$/', $zone)) {
                $zone .= LatitudeLongitude::UTMLetterDesignator($zone);
            }

            $this->utmZone             = $zone;
            $this->latitude_of_origin  = $this->latitudeToZoneLetter(substr($zone, -1));
    ...
    {
        return LatitudeLongitude::zoneNumber($this->latitude_of_origin, $this->longitude_of_origin)
           . LatitudeLongitude::UTMLetterDesignator($this->latitude_of_origin);
    }
