<?php
defined('ANS') or die();

$Data->execute('acl-action.php', array('action' => 'importar-observacions'));

$resultado = array();
$arquivo = $Vars->var['arquivo'];

if ($arquivo['tmp_name'] && is_file($arquivo['tmp_name'])) {
    if (strtolower(end(explode('.', $arquivo['name']))) !== 'csv') {
        $Vars->message(__('El fichero que has enviado no parece ser fichero CSV válido, revisa que la extensión del mismo sea "csv".'), 'ko');
        return false;
    }

    $finfo = finfo_open(FILEINFO_MIME);

    if (substr(finfo_file($finfo, $arquivo['tmp_name']), 0, 4) !== 'text') {
        $Vars->message(__('El fichero que has enviado no parece ser fichero CSV válido.'), 'ko');
        return false;
    }

    $arquivo = file($arquivo['tmp_name'], FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);

    $header = array_shift($arquivo);
} else {
    $arquivo = array();
}

use ANS\PHPCan\Loader;

Loader::registerComposer(SCENE_PATH.'libs/willdurand/Geocoder/vendor/');

$Geocoder = new \Geocoder\Geocoder();
$Geocoder->registerProviders(array(
    new \Geocoder\Provider\OpenStreetMapsProvider(
        new \Geocoder\HttpAdapter\CurlHttpAdapter()
    )
));

/*
Array
(
    [0] => ESPECIES
    [1] => Localidade
    [2] => UTM MGRS
    [3] => FUSO
    [4] => UTM_X
    [5] => UTM_Y
    [6] => Lat
    [7] => Long
    [8] => Datum
    [9] => Tipo de Dado
    [10] => Arquivo
    [11] => Tipo de referencia
    [12] => Referencia bibliográfica
    [13] => Fornecedor de Datos
    [14] => Observacións
    [15] => Data da observación
    [16] => Outros observadores
    [17] => Estado de conservación
    [18] => Sexo
    [19] => Fase ou estadío
    [20] => Comportamento migratorio
    [21] => Especies acompañantes
)
*/

$puntosInvalidos = 0;
$listado = $especie = array();
foreach ($arquivo as $i => $fila) {
    $csv = trimArray(str_getcsv($fila, ',', '"'));

    if (empty($csv[0])) {
        continue;
    }

    $csv = str_getcsv($fila, ',', '"');

    $code = str_replace('_', '', $csv[0]);
    $code = alphaNumeric($code);

    if (empty($listado[$code])) {
        $listado[$code] = array(
            'i' => $i,
            'nome' => $csv[0],
            'especie' => array(),
            'fichas' => array(),
            'puntosInvalidos' => 0
        );
    }

    $md5 = $csv;

    unset($md5[2], $md5[3], $md5[4], $md5[5], $md5[6], $md5[7], $md5[8]);

    $md5 = md5(serialize($md5));

    $puntoValido = false;

    if ($csv[2] || ($csv[3] && $csv[4] && $csv[5]) || ($csv[6] && $csv[7])) {
        $puntoValido = true;
    }

    if (empty($listado[$code]['fichas'][$md5])) {

        $exists = $Db->select(array(
            'table' => 'avistamentos',
            'fields' => 'url',
            'limit' => 1,
            'conditions' => array(
                'checksum' => $md5
            )
        ));

        /*
        Campos selectores: sexo (db enum), fase (db enum), migracion (db enum), especies_acompanhantes (db relation)
        */

        $opcionsSexo = array('', 'macho', 'femia');
        $opcionsFase = array('', 'ovo', 'larva', 'ninfa', 'cria', 'adulto', 'outros');
        $opcionsMigracion = array('', 'invernante', 'pasaxe', 'estivais', 'residentes');

        $listado[$code]['fichas'][$md5] = array(
            'localidade' => $csv[1],
            'tipo' => $csv[9],
            'checksum' => $md5,
            'exists' => $exists,
            'arquivo' => $csv[10],
            'tipo_referencia' => $csv[11],
            'referencia' => $csv[12],
            'fornecedor' => $csv[13],
            'observacions' => $csv[14],
            'data_observacion' => $csv[15],
            'outros_observadores' => $csv[16],
            'sexo' => $csv[18],
            'opcions_sexo' => $opcionsSexo,
            'fase' => $csv[19],
            'opcions_fase' => $opcionsFase,
            'migracion' => $csv[20],
            'opcions_migracion' => $opcionsMigracion,
            'especies_acompanhantes' => $csv[21],
            'puntosInvalidos' => 0,
            'puntos' => array(array(
                'mgrs' => $csv[2],
                'fuso' => $csv[3],
                'utm_x' => $csv[4],
                'utm_y' => $csv[5],
                'lat' => $csv[6],
                'long' => $csv[7],
                'datum' => $csv[8],
                'valido' => $puntoValido
            ))
        );
    } else {
        $listado[$code]['fichas'][$md5]['puntos'][] = array(
            'mgrs' => $csv[2],
            'fuso' => $csv[3],
            'utm_x' => $csv[4],
            'utm_y' => $csv[5],
            'lat' => $csv[6],
            'long' => $csv[7],
            'datum' => $csv[8],
            'valido' => $puntoValido
        );
    }

    if (!$puntoValido) {
        $listado[$code]['puntosInvalidos']++;
        $listado[$code]['fichas'][$md5]['puntosInvalidos']++;
        $puntosInvalidos++;
    }
    
}

foreach ($listado as &$fila) {
    $nome = '+'.implode(' +', explodeTrim(' ', str_replace('"', '', $fila['nome'])));

    $query = 'SELECT `id`, `url`, `nome`, MATCH(`nome`, `sinonimos`) AGAINST ("'.$nome.'" IN BOOLEAN MODE) AS `puntos` FROM `especies`'
        .' WHERE MATCH(`nome`, `sinonimos`) AGAINST ("'.$nome.'" IN BOOLEAN MODE);';

    $especie = $Db->queryResult($query);

    if (empty($especie)) {
        $nome = explodeTrim(' ', $nome);

        $query = 'SELECT `id`, `url`, `nome`, MATCH(`nome`, `sinonimos`) AGAINST ("'.$nome[0].'" IN BOOLEAN MODE) AS `puntos` FROM `especies`'
            .' WHERE MATCH(`nome`, `sinonimos`) AGAINST ("'.$nome[0].'" IN BOOLEAN MODE);';

        $especie = $Db->queryResult($query);
    }

    /*
    $fila['checksum'] = md5(serialize($fila));

    $fila['exists'] = $Db->select(array(
        'table' => 'avistamentos',
        'fields' => 'url',
        'limit' => 1,
        'conditions' => array(
            'checksum' => $fila['checksum']
        )
    ));
    */

    if (empty($especie)) {
    	
    	$fila['status'] = 'new';
        $fila['statusName'] = __('Nova');
    	$fila['message'] = __('Non se atopoua a especie, se non selecciona ningunha darase de alta como unha nova');
    	$especie = array();
    } else if (count($especie) > 1){
    	$fila['status'] = 'unknown';
        $fila['statusName'] = __('Multiple');
    	$fila['message'] = __('Atopáronse varias posibilidades para esta observación, debe escoller unha');
    } else {
    	$fila['status'] = 'success';
        $fila['statusName'] = __('Atopada');
    	$fila['message'] = __('Confirma que a especie que atopamos é correcto ou escolle unha nova.');
    }

    $fila['especie'] = $especie;    
}

//pre($listado);

$resultado = $listado;
