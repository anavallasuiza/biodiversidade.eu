...
<div id="<?php echo $url; ?>" class="datos-avistamento <?php echo $css ?: ''; ?>">
	<article class="especie-avistamento mediano" data-codigo="<?php echo $url; ?>" data-especie="<?php echo $especie_url; ?>">
		<figure class="especie-imaxe">
			<?php
			echo $Html->img(array(
				'src' => ($imaxe ? ('uploads|'.$imaxe) : 'templates|img/logo-imaxe.png'),
		...
		<header>
			<h1>
				<a href="<?php echo path('avistamento', $url); ?>">
                    <?php 
                    if ($especie_nome) {
                        echo $especie_nome . ' - ' . $especie_grupo . ' ' . $especie_familia; 
                    } else { 
				...
				<?php if ($validado) { ?>
				<span class="estado solucionada"<?php echo $usuarios_validador_nome ? (' title="'.__('Validada por %s', $usuarios_validador_nome).'"') : ''; ?>><i class="icon-thumbs-up"></i> <?php __e('Validada'); ?></span>
				<?php } else { ?>
			...
			<ul class="avistamento-informacion">
				...
				<li title="<?php __e('Usuario que diud e alta a observación'); ?>">
					<i class="icon-user"></i>
					<a href="<?php echo path('perfil', $usuarios_autor_url); ?>">
						<?php echo $usuarios_autor_nome.' '.$usuarios_autor_apelido1; ?>
					</a>
		...
		<footer>
			<a href="<?php echo path('especie', $especie_url); ?>"><?php __e('Ficha da especie'); ?></a>
			<a href="<?php echo path('avistamento', $url); ?>"><?php __e('Ficha da observacion'); ?></a>

			<?php if ((EDITOR === true) || ($user && ($usuarios_autor === $user['id']))) {?>
